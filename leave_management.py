...
def get_employee_leave_balance(employee_id, leave_type_id, year=None):
    ...
    ''', (employee_id, leave_type_id, year), fetch='one')
...
def create_leave_balance_for_employee(employee_id, year=None):
    ...
    leave_types = execute_query('SELECT * FROM leave_types', fetch='all')
    ...
    for leave_type in leave_types:
        ...
        existing = execute_query('''
            SELECT * FROM employee_leave_balance
            WHERE employee_id = ? AND leave_type_id = ? AND year = ?
        ''', (employee_id, leave_type['id'], year), fetch='one')
        ...
        if not existing:
            ...
            insert_data.append((
                ...
                leave_type['max_days'],
                0,  # used_days
                leave_type['max_days']   # remaining_days
            ))
